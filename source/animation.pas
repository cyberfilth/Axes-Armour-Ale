...
begin
  ...
  UnlockScreenUpdate;
  UpdateScreen(False);
  ui.displayMessage(entityList[id].race + ' casts necromantic energy');
  for i := 2 to 10 do
  begin
    LockScreenUpdate;
    if (flightPath[i].X <> 0) and (flightPath[i].Y <> 0) then
    begin
      (* Paint over previous blast *)
      if (i > 2) then
      begin
        map.mapDisplay[flightPath[i - 1].Y, flightPath[i - 1].X].GlyphColour := 'magenta';
        map.mapDisplay[flightPath[i - 1].Y, flightPath[i - 1].X].Glyph := '*';
      end;
      (* Draw blast *)
      map.mapDisplay[flightPath[i].Y, flightPath[i].X].GlyphColour := 'lightMagenta';
  ...
  LockScreenUpdate;
  (* Paint out NPC to fix a glitch with updating the display *)
  TextOut(flightPath[1].X, flightPath[1].Y, 'darkGrey', '.');
  (* Repaint map *)
  camera.drawMap;
  fov.fieldOfView(entityList[0].posX, entityList[0].posY, entityList[0].visionRange, 1);
  UnlockScreenUpdate;
