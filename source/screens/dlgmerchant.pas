...
begin
  ...
  begin
    ...
    {$IFDEF Windows}
    TextOut(5, y, BG, ' ');
    TextOut(6 + Length('Buy ' + itmType), y, BG, ' ');
    {$ENDIF}
    (* Vertical sides *)
    for y := 6 to 10 do
      TextOut(3, y, BG, chr(186) + '                                                  ' + chr(186));
...
begin
  ...
  (* End borders around title *)
    ...
    {$IFDEF Windows}
    TextOut(5, y, BG, ' ');
    TextOut(6 + Length('Buy or Sell?'), y, BG, ' ');
    {$ENDIF}
  (* Vertical sides *)
  for y := 6 to 8 do
    TextOut(3, y, BG, chr(186) + '                                                  ' + chr(186));
...
begin
  ...
  begin
    ...
    {$IFDEF Windows}
    TextOut(5, y, BG, ' ');
    TextOut(6 + Length(' Select item from your inventory '), y, BG, ' ');
    {$ENDIF}
    (* Vertical sides *)
    for y := 6 to (8 + totalEntries) do
      TextOut(3, y, BG, chr(186) + '                                                  ' + chr(186));
    ...
    begin
      if (player_inventory.inventory[i].itemType = itmDrink) or
        ...
        (player_inventory.inventory[i].itemType = itmArmour) then
      begin
        if (player_inventory.inventory[i].equipped = False) then
        begin
          TextOut(5, j, FG, '[' + IntToStr(i) + '] ' +
            player_inventory.inventory[i].Name + ' - sale value $' + IntToStr(saleValue(player_inventory.inventory[i].Value)));
          Inc(j);
        end;
      end;
    end;
...
begin
  ...
  if (player_inventory.inventory[selection].itemType = itmDrink) then
    itmType := 'drink'
  else if (player_inventory.inventory[selection].itemType = itmWeapon) or
    (player_inventory.inventory[selection].itemType = itmProjectileWeapon) then
    itmType := 'weapon'
  ...
  {$IFDEF Windows}
  TextOut(5, y, BG, ' ');
  TextOut(6 + Length('Confirm sale of ' + itmType), y, BG, ' ');
  {$ENDIF}
  (* Vertical sides *)
  for y := 6 to 10 do
    TextOut(3, y, BG, chr(186) + '                                                  ' + chr(186));
  ...
  Inc(y);
  (* List item *)
  TextOut(5, 7, player_inventory.inventory[selection].glyphColour,
    player_inventory.inventory[selection].glyph);
