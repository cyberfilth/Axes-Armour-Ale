...
uses
  globalUtils, universe, map, smell, village,
  { List of creatures }
...
const
  ...
  stoneCavernUnique3: array[1..2] of string = ('trogGiant', 'trogGiant');
 ...
 (* TESTING CODE - villagers. This code will be removed *)
  villageNPCs: array[1..5] of string = ('villager', 'villager', 'villager', 'villager', 'villager');

var
  r, c, shackLocation: smallint;
  randSelect: byte;
  monster: string;
begin
  r := 0;
  ...
  begin
    ...
    for shackLocation := 0 to high(village.centreList) do
    begin
      r := village.centreList[shackLocation].y;
      c := village.centreList[shackLocation].x;
    end;
    randSelect := globalUtils.randomRange(1, Length(villageNPCs));
    monster := villageNPCs[randSelect];
