...
procedure createCaveBear(uniqueid, npcx, npcy: smallint);
begin
  (* Add a bear to the list of creatures *)
...
begin
  ...
  begin
    ...
    begin
      ui.displayMessage(entityList[id].race + ' howls and attacks itself');
      Dec(entityList[id].currentHP);
    end;
...
begin
    ...
    begin
      ...
      begin
        if (enemyID = 0) then
          ...
          ui.displayMessage('The cave bear slashes you')
        else
          (* If an NPC is slightly wounded *)
          ui.displayMessage('The cave bear slightly wounds the ' + entityList[enemyID].race);
      end
      else
      ...
      begin
        if (enemyID = 0) then
          (* To the player *)
          begin
            ui.displayMessage('The cave bear bites you, inflicting ' + IntToStr(damageAmount) + ' damage');
            (* Update health display to show damage *)
            ui.updateHealth;
          end
        else
          (* To an NPC *)
          ui.displayMessage('The cave bear bites the ' + entityList[enemyID].race);
      end;
    end;
  end
  else
  ...
  begin
    if (enemyID = 0) then
    begin
      ui.displayMessage('The cave bear roars');
      combat_resolver.spiteDMG(npcID);
    end
    else
      ui.displayMessage('The cave bear nips at the ' + entityList[enemyID].race + ', but misses');
  end;
end;
