...
uses
  SysUtils, Math, smell, universe, combat_resolver, items, terbutje;

(* Create a Troglodyte *)
procedure createGiantTroglodyte(uniqueid, npcx, npcy: smallint);
(* Take a turn *)
...
begin
  ...
  begin
    ...
    article := True;
    if (desc = 1) then
      description := 'a giant Troglodyte'
    else if (desc = 2) then
      description := 'a massive Troglodyte'
    else
      description := 'a towering Troglodyte';
    glyph := chr(2);
    glyphColour := 'lightMagenta';
    maxHP := randomRange(8, 9) + universe.currentDepth;
    currentHP := maxHP;
    attack := randomRange(8, 10);
    defence := randomRange(6, 8);
    weaponDice := 0;
    weaponAdds := 0;
    xpReward := maxHP;
    visionRange := 5;
...
begin
  ...
  begin
    ...
    terbutje.createTerbutje(entityList[id].posX, entityList[id].posY);
    ui.displayMessage('The Trog giant drops a terbutje');
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      if (damageAmount = 1) then
        ui.displayMessage('The Trog giant slightly wounds you')
      else
        ui.displayMessage('The Trog giant hits you, dealing ' + IntToStr(damageAmount) + ' damage');
  ...
  begin
    ...
    if (chance = 1) then
      ui.displayMessage('The Trog giant attacks wildly but misses')
    else if (chance = 2) then
      ui.displayMessage('The Trog giant flails at you');
