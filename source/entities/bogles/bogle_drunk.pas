...
uses
  SysUtils, Math, smell, globalUtils, universe, combat_resolver, player_stats,
  ale_tankard, items;

(* Create a Bogle *)
procedure createDrunkBogle(uniqueid, npcx, npcy: smallint);
(* Take a turn *)
...
procedure createDrunkBogle(uniqueid, npcx, npcy: smallint);
begin
  (* Add a Bogle to the list of creatures *)
...
begin
  ...
  begin
    ...
    begin
      ui.displayMessage(entityList[id].race + ' trips over');
      Dec(entityList[id].currentHP);
    end;
...
begin
  ...
  begin
    ...
    begin
      ui.bufferMessage('The Bogle bumps into ' + getCreatureName(newX, newY));
      entities.moveNPC(id, spx, spy);
    end
...
begin
  ...
  begin
    ...
    if (chance = 1) then
      ui.displayMessage('The Bogle screams curses')
    else if (chance = 2) then
      ui.displayMessage('The Bogle drunkenly yells');
